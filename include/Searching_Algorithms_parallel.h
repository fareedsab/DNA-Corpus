...
long long unsigned int Calculate_Hashcode(string &Pattern) 
{
   ...
   // #pragma omp parallel shared(i,k,Hashcode,Pattern) 
    //{
        while (i>-1) 
        {
...
int Rabin_Karp_Algorithm(string &Data, string &Pattern, int Start = 0) 
{
    ...
    long long unsigned int Hashcode_Of_Sliding_Window;
     omp_set_num_threads(64);
   #pragma omp parallel shared (i,k,Sliding_Window,Hashcode_Of_Pattern,Start)
    {
        while (i<Length)  
        {
        ...
        Hashcode_Of_Sliding_Window = Calculate_Hashcode(Sliding_Window);
    }
...
//        #pragma omp parallel shared(i,original, __searchQuery)
//        {
...
void searchFromFileKM(string __searchQuery) 
{
    ...
    while (inputFile >> input) 
    {
        string original = input;
        transform(input.begin(), input.end(), input.begin(), ::toupper);
     //   #pragma omp parallel shared (input,__searchQuery,i,original)
     //{
		   while (i < input.length()) 
        {
